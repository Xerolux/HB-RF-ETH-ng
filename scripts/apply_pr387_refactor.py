...
text = text.replace("import { useExperimentalStore, useSettingsStore, useLoginStore, useUiStore, useSysInfoStore, useRestartUiStore } from './stores.js'", "import { useSettingsStore, useLoginStore, useUiStore, useSysInfoStore, useRestartUiStore } from './stores.js'")
...
text = read(path)
text = re.sub(r"export const useExperimentalStore = defineStore\('experimental', \{.*?\n\}\)\n\nexport const useLoginStore", """export const useExperimentalStore = defineStore('experimental', {
...
export const useLoginStore""", text, count=1, flags=re.S)
write(path, text)

path = "main/settings.cpp"
text = read(path)
text = text.replace('  GET_BOOL(handle, "flashPause", _flashPause, false);\n  GET_BOOL(handle, "testDesign", _testDesignEnabled, false);', '  // Fixed defaults for all devices; legacy false values are ignored.\n  _flashPause = true;\n  _testDesignEnabled = true;')
text = text.replace('  SET_BOOL(handle, "flashPause", _flashPause);\n  SET_BOOL(handle, "testDesign", _testDesignEnabled);', '  SET_BOOL(handle, "flashPause", true);\n  SET_BOOL(handle, "testDesign", true);')
...
if "experimentalEmptyTitle" not in text:
    text = replace_once(text, anchor, anchor + "\n    experimentalEmptyTitle: 'Derzeit nichts vorhanden',\n    experimentalEmptyText: 'Aktuell sind keine experimentellen Funktionen verfügbar. Das New Design und der Neustart-Sync sind feste Standards für alle Geräte.',", path)
write(path, text)

# Independent WebUI version and archive removal.
path = "main/CMakeLists.txt"
text = read(path)
text = text.replace('# Embedded firmware release archive. Regenerate with scripts/update_archive.py\n# whenever archive.json changes (release process). Served from flash by\n# /api/firmware_archive so the WebUI never has to fetch it live from GitHub.\ntarget_add_binary_data(${COMPONENT_TARGET} "generated/archive.json.gz" BINARY)\n\n', '')
...
if "#ifndef HB_WEBUI_VERSION" not in text:
    text = text.replace('extern esp_err_t validate_auth(httpd_req_t *req);', 'extern esp_err_t validate_auth(httpd_req_t *req);\n\n#ifndef HB_WEBUI_VERSION\n#define HB_WEBUI_VERSION "unknown"\n#endif')
...
if "void webui_storage_get_effective_version" not in text:
    text = replace_once(text, status_marker, status_marker + '''
void webui_storage_get_effective_version(char *output, size_t outputSize)
{
    if (!output || outputSize == 0) return;
    StorageLock lock;
    if (!lock) { copy_safe(output, outputSize, HB_WEBUI_VERSION); return; }
    copy_safe(output, outputSize, s_status.valid && s_status.version[0] ? s_status.version : HB_WEBUI_VERSION);
}
''', path)
text = text.replace('    char safe_version[sizeof(status.version)] = {};\n    char safe_error[sizeof(status.lastError)] = {};\n    copy_json_safe(safe_version, sizeof(safe_version), status.version);', '    char safe_version[sizeof(status.version)] = {};\n    char effective_version[sizeof(status.version)] = {};\n    char safe_error[sizeof(status.lastError)] = {};\n    copy_json_safe(safe_version, sizeof(safe_version), status.version);\n    webui_storage_get_effective_version(effective_version, sizeof(effective_version));')
text = text.replace('\"usedBytes\":%u,\"bytesWritten\":%u,\"version\":\"%s\",\n             \"design\":\"newdesign\",\"lastError\":\"%s\"}', '\"usedBytes\":%u,\"bytesWritten\":%u,\"version\":\"%s\",\n             \"effectiveVersion\":\"%s\",\"design\":\"newdesign\",\n             \"lastError\":\"%s\"}')
text = text.replace('              safe_version,\n              safe_error);', '              safe_version,\n              effective_version,\n              safe_error);')
text = text.replace('set_error_locked("WWW image size does not match SPIFFS partition");', 'set_error_locked("Falsche WebUI-Datei: Erwartet wird ein 327680-Byte-WWW-Image (webui_*.bin oder spiffs.bin)");')
write(path, text)

path = "main/system_overview_api.cpp"
text = read(path)
if "webui_effective_version" not in text:
    text = text.replace('    const WebUIStorageStatus webui = webui_storage_get_status();', '    const WebUIStorageStatus webui = webui_storage_get_status();\n    char webui_effective_version[32] = {};\n    webui_storage_get_effective_version(webui_effective_version, sizeof(webui_effective_version));')
text = text.replace('        cJSON_AddStringToObject(webui_object, "version",\n                                webui.version[0] ? webui.version : "embedded");', '        cJSON_AddStringToObject(webui_object, "version",\n                                webui_effective_version);')
write(path, text)

path = "webui/src/app.vue"
text = read(path)
text = replace_once(text, "          <small class=\"text-muted\">{{ t('app.footerCopyright', { version: sysInfoStore.currentVersion ? 'v' + sysInfoStore.currentVersion : '' }) }}</small>", "          <small class=\"text-muted\">Firmware v{{ sysInfoStore.currentVersion || '—' }} · WebUI v{{ webUiVersion }} © 2025-2026 Xerolux</small>", path)
...
if proxy_start >= 0 and proxy_end > proxy_start: text = text[:proxy_start] + text[proxy_end:]
...
if archive_start >= 0 and archive_end > archive_start: text = text[:archive_start] + text[archive_end:]
for line in ['        httpd_register_uri_handler(_httpd_handle, &post_ota_url_handler);\n', '        httpd_register_uri_handler(_httpd_handle, &get_changelog_handler);\n', '        httpd_register_uri_handler(_httpd_handle, &get_firmware_archive_handler);\n']:
    text = text.replace(line, '')
for inc in ['#include "esp_https_ota.h"\n', '#include "esp_crt_bundle.h"\n', '#include "ota_config.h"\n']:
...
if "Firmwarekopf 0xE9" not in text:
    text = replace_once(text, magic_anchor, '            if (recv_len < 1 || static_cast<unsigned char>(ota_buff[0]) != 0xE9)\n            {\n                httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST,\n                    "Ungueltige Firmware-Datei: Der ESP32-Firmwarekopf 0xE9 fehlt. WebUI-Dateien unter System -> WebUI installieren.");\n                goto err;\n            }\n\n' + magic_anchor, path)
write(path, text)

# MQTT reports two versions and never installs.
path = "main/mqtt_handler.cpp"
text = read(path)
if '#include "webui_storage.h"' not in text: text = text.replace('#include "updatecheck.h"', '#include "updatecheck.h"\n#include "webui_storage.h"')
text = re.sub(r"static constexpr uint32_t MQTT_UPDATE_TASK_STACK_BYTES = \d+;\n\n", "", text)
text = re.sub(r"\s*\} else if \(strcmp\(command, \"update\"\) == 0\) \{.*?\n\s*\} else \{", "\n    } else {", text, count=1, flags=re.S)
...
new_update = '''    // ---- Informational update status -------------------------------------
    if (updateCheck) {
        ...
        if (rel.valid && compareVersions(latestFirmware, currentVersion) <= 0) latestFirmware = currentVersion;
        const bool firmwareUpdateAvailable = rel.valid && compareVersions(rel.version, currentVersion) > 0;
        const char* latestWebui = rel.webuiValid ? rel.webuiVersion : webuiVersion;
        const bool webuiUpdateAvailable = rel.webuiValid && compareVersions(rel.webuiVersion, webuiVersion) > 0;
