...
<script>
let token='';const $=id=>document.getElementById(id);const status=m=>$('status').textContent=m;
...
$('restart').onclick=async()=>{if(!confirm('Gerät wirklich neu starten?'))return;try{await api('/api/restart',{method:'POST'});status('Neustart wurde ausgelöst.')}catch(e){status('Neustart ausgelöst; Verbindung wird getrennt.')}};
