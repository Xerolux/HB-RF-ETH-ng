...
static bool backup_get_uint(const cJSON *object, const char *key, unsigned maximum, unsigned *value)
{
    const cJSON *item = cJSON_GetObjectItemCaseSensitive(object, key);
    if (!cJSON_IsNumber(item) || !value || item->valuedouble < 0 || item->valuedouble > maximum ||
...
static bool backup_get_string(const cJSON *object, const char *key, char *value, size_t value_size)
{
    const cJSON *item = cJSON_GetObjectItemCaseSensitive(object, key);
    if (!cJSON_IsString(item) || !item->valuestring || !value || value_size == 0 ||
...
{
    monitoring_config_t *config = new (std::nothrow) monitoring_config_t();
    ...
    cJSON *checkmk    = monitoring ? cJSON_AddObjectToObject(monitoring, "checkmk") : NULL;
    cJSON *mqtt       = monitoring ? cJSON_AddObjectToObject(monitoring, "mqtt") : NULL;
    cJSON *prometheus = monitoring ? cJSON_AddObjectToObject(monitoring, "prometheus") : NULL;
    cJSON *syslog     = monitoring ? cJSON_AddObjectToObject(monitoring, "syslog") : NULL;
    cJSON *notify     = monitoring ? cJSON_AddObjectToObject(monitoring, "notify") : NULL;
    if (!monitoring || !checkmk || !mqtt || !prometheus || !syslog || !notify) {
        delete config;
    ...
    cJSON_AddStringToObject(mqtt, "tlsCaCerts", config->mqtt.tls_ca_certs);
    cJSON_AddStringToObject(mqtt, "tlsCertfile", config->mqtt.tls_certfile);
    cJSON_AddStringToObject(mqtt, "tlsKeyfile", config->mqtt.tls_keyfile);
    cJSON_AddBoolToObject(mqtt, "commandEnabled", config->mqtt.command_enabled);
    cJSON_AddStringToObject(mqtt, "commandToken", config->mqtt.command_token);

    cJSON_AddBoolToObject(prometheus, "enabled", config->prometheus.enabled);
    cJSON_AddNumberToObject(prometheus, "port", config->prometheus.port);
    cJSON_AddStringToObject(prometheus, "allowedHosts", config->prometheus.allowed_hosts);
    ...
    cJSON_AddStringToObject(notify, "webhookSecret", config->notify.webhook_secret);
    cJSON_AddStringToObject(notify, "telegramToken", config->notify.telegram_token);
    ...
    cJSON_AddStringToObject(notify, "smtpPassword", config->notify.smtp_password);
...
static esp_err_t parse_monitoring_backup(const cJSON *root, monitoring_config_t **restored_config)
{
    if (!restored_config) return ESP_ERR_INVALID_ARG;
    *restored_config = NULL;

    const cJSON *monitoring = cJSON_GetObjectItemCaseSensitive(root, "monitoring");
    if (!monitoring) return ESP_OK; // Backwards-compatible legacy backup.
    if (!cJSON_IsObject(monitoring)) return ESP_ERR_INVALID_ARG;

    const cJSON *checkmk    = cJSON_GetObjectItemCaseSensitive(monitoring, "checkmk");
    const cJSON *mqtt       = cJSON_GetObjectItemCaseSensitive(monitoring, "mqtt");
    const cJSON *prometheus = cJSON_GetObjectItemCaseSensitive(monitoring, "prometheus");
    const cJSON *syslog     = cJSON_GetObjectItemCaseSensitive(monitoring, "syslog");
    const cJSON *notify     = cJSON_GetObjectItemCaseSensitive(monitoring, "notify");
    if (!cJSON_IsObject(checkmk) || !cJSON_IsObject(mqtt) || !cJSON_IsObject(prometheus) ||
        !cJSON_IsObject(syslog) || !cJSON_IsObject(notify)) {
    ...
    bool valid = backup_get_bool(checkmk, "enabled", &config->checkmk.enabled) &&
    ...
    valid = valid && validatePort(config->checkmk.port) &&
            backup_get_string(checkmk, "allowedHosts", config->checkmk.allowed_hosts,
                              sizeof(config->checkmk.allowed_hosts));

    valid = valid && backup_get_bool(mqtt, "enabled", &config->mqtt.enabled) &&
            backup_get_string(mqtt, "server", config->mqtt.server, sizeof(config->mqtt.server)) &&
            backup_get_uint(mqtt, "port", UINT16_MAX, &number);
    if (valid) config->mqtt.port = static_cast<uint16_t>(number);
    valid =
        valid && validatePort(config->mqtt.port) &&
        backup_get_string(mqtt, "user", config->mqtt.user, sizeof(config->mqtt.user)) &&
        backup_get_string(mqtt, "password", config->mqtt.password, sizeof(config->mqtt.password)) &&
        backup_get_string(mqtt, "topicPrefix", config->mqtt.topic_prefix,
                          sizeof(config->mqtt.topic_prefix)) &&
        backup_get_bool(mqtt, "haDiscoveryEnabled", &config->mqtt.ha_discovery_enabled) &&
        backup_get_string(mqtt, "haDiscoveryPrefix", config->mqtt.ha_discovery_prefix,
                          sizeof(config->mqtt.ha_discovery_prefix)) &&
        backup_get_bool(mqtt, "tlsEnable", &config->mqtt.tls_enable) &&
        backup_get_bool(mqtt, "tlsSkipVerify", &config->mqtt.tls_skip_verify) &&
        backup_get_string(mqtt, "tlsCaCerts", config->mqtt.tls_ca_certs,
                          sizeof(config->mqtt.tls_ca_certs)) &&
        backup_get_string(mqtt, "tlsCertfile", config->mqtt.tls_certfile,
                          sizeof(config->mqtt.tls_certfile)) &&
        backup_get_string(mqtt, "tlsKeyfile", config->mqtt.tls_keyfile,
                          sizeof(config->mqtt.tls_keyfile)) &&
        backup_get_bool(mqtt, "commandEnabled", &config->mqtt.command_enabled) &&
        backup_get_string(mqtt, "commandToken", config->mqtt.command_token,
                          sizeof(config->mqtt.command_token));
    if (valid && (config->mqtt.enabled || config->mqtt.server[0] != '\0')) {
        valid = validateServerAddress(config->mqtt.server, sizeof(config->mqtt.server) - 1);
    }
    if (valid && config->mqtt.command_token[0] != '\0') {
        valid = validateMqttCommandToken(config->mqtt.command_token);
    }

    valid = valid && backup_get_bool(prometheus, "enabled", &config->prometheus.enabled) &&
    ...
    valid = valid && validatePort(config->prometheus.port) &&
            backup_get_string(prometheus, "allowedHosts", config->prometheus.allowed_hosts,
                              sizeof(config->prometheus.allowed_hosts));

    valid =
        valid && backup_get_bool(syslog, "enabled", &config->syslog.enabled) &&
        backup_get_string(syslog, "server", config->syslog.server, sizeof(config->syslog.server)) &&
    ...
    valid = valid && backup_get_uint(syslog, "minSeverity", 7, &number);
    if (valid) config->syslog.min_severity = static_cast<uint8_t>(number);
    valid = valid && backup_get_string(syslog, "hostname", config->syslog.hostname,
                                       sizeof(config->syslog.hostname));
    if (valid && (config->syslog.enabled || config->syslog.server[0] != '\0')) {
        valid = validateServerAddress(config->syslog.server, sizeof(config->syslog.server) - 1);
    }

    valid = valid && backup_get_bool(notify, "enabled", &config->notify.enabled) &&
            backup_get_uint(notify, "channels", 7, &number);
    if (valid) config->notify.channels = static_cast<uint8_t>(number);
    valid = valid &&
            backup_get_string(notify, "webhookUrl", config->notify.webhook_url,
                              sizeof(config->notify.webhook_url)) &&
            backup_get_string(notify, "webhookSecret", config->notify.webhook_secret,
                              sizeof(config->notify.webhook_secret)) &&
            backup_get_string(notify, "telegramToken", config->notify.telegram_token,
                              sizeof(config->notify.telegram_token)) &&
            backup_get_string(notify, "telegramChatId", config->notify.telegram_chatid,
                              sizeof(config->notify.telegram_chatid)) &&
            backup_get_string(notify, "smtpServer", config->notify.smtp_server,
    ...
    valid = valid &&
            ...
            backup_get_string(notify, "smtpPassword", config->notify.smtp_password,
...
static esp_err_t add_theme_backup(cJSON *root)
{
    char scheme[8]   = {};
    char color[8]    = {};
    esp_err_t result = theme_api_get_config(scheme, sizeof(scheme), color, sizeof(color));
...
static esp_err_t parse_theme_backup(const cJSON *root, char scheme[8], char color[8],
                                    bool *has_theme)
{
    ...
    *has_theme         = false;
    ...
    if (!cJSON_IsObject(theme) || !backup_get_string(theme, "colorScheme", scheme, 8) ||
        !backup_get_string(theme, "primaryColor", color, 8)) {
        return ESP_ERR_INVALID_ARG;
    }
    if (strcmp(scheme, "system") != 0 && strcmp(scheme, "light") != 0 &&
...
esp_err_t get_backup_handler_func(httpd_req_t *req)
{
    ...
    cJSON_AddStringToObject(portability, "warning",
    ...
    // If we want a flat structure or specific structure for restore, we need to match
    // post_settings_json_handler expectations. post_settings_json_handler expects a flat JSON
    // object with keys like "adminUsername", "adminPassword", "hostname", etc. But add_settings
    // creates { "settings": { "hostname": ... } }
    ...
    cJSON_AddStringToObject(root, "adminPassword", webui_settings()->getAdminPassword());
    cJSON_AddBoolToObject(root, "passwordChanged", webui_settings()->getPasswordChanged());

    if (add_theme_backup(root) != ESP_OK || add_monitoring_backup(root) != ESP_OK) {
        cJSON_Delete(root);
...
esp_err_t post_restore_handler_func(httpd_req_t *req)
{
    ...
    const esp_err_t monitoring_parse_result = parse_monitoring_backup(root, &parsed_monitoring);
    std::unique_ptr<monitoring_config_t> restored_monitoring(parsed_monitoring);
    char restored_theme_scheme[8] = {};
    char restored_theme_color[8]  = {};
    bool has_restored_theme       = false;
    const esp_err_t theme_parse_result =
        parse_theme_backup(root, restored_theme_scheme, restored_theme_color, &has_restored_theme);
    if (monitoring_parse_result != ESP_OK || theme_parse_result != ESP_OK) {
        cJSON_Delete(root);
        return send_json_error(req, "400 Bad Request", "invalid_backup", "backup");
    }
    ...
    cJSON *passwordChangedItem = cJSON_GetObjectItem(root, "passwordChanged");
    ...
    int dcfOffset = cJSON_GetIntValueSafe(cJSON_GetObjectItem(root, "dcfOffset"),
                                          webui_settings()->getDcfOffset());

    int gpsBaudrate = cJSON_GetIntValueSafe(cJSON_GetObjectItem(root, "gpsBaudrate"),
                                            webui_settings()->getGpsBaudrate());

    char *ntpServer      = cJSON_GetStringValue(cJSON_GetObjectItem(root, "ntpServer"));
    int ledBrightness    = cJSON_GetIntValueSafe(cJSON_GetObjectItem(root, "ledBrightness"),
                                                 webui_settings()->getLEDBrightness());
    ...
    int ipv6PrefixLength = cJSON_GetIntValueSafe(cJSON_GetObjectItem(root, "ipv6PrefixLength"), 64);
    ...
    const bool has_password_changed_flag = passwordChangedItem && cJSON_IsBool(passwordChangedItem);
    if (adminUsername && adminUsername[0] != '\0' && !valid_admin_username(adminUsername)) {
        cJSON_Delete(root);
        return send_json_error(req, "400 Bad Request", "invalid_username", "adminUsername");
    }
    ...
    if (adminPassword && adminPassword[0] != '\0') {
        ...
        if (password_len >= 33) {
            cJSON_Delete(root);
            return send_json_error(req, "400 Bad Request", "invalid_password", "adminPassword");
        }
        admin_password_change_requested =
            secure_strcmp(adminPassword, webui_settings()->getAdminPassword()) != 0;
        ...
        const bool password_valid =
            !admin_password_change_requested ||
            (has_password_changed_flag ? password_len > 0 : validateAdminPassword(adminPassword));
        if (!password_valid) {
            cJSON_Delete(root);
            return send_json_error(req, "400 Bad Request", "invalid_password", "adminPassword");
        }
    ...
    if (admin_password_change_requested) {
        if (currentPassword == NULL || currentPassword[0] == '\0') {
            cJSON_Delete(root);
            return send_json_error(req, "400 Bad Request", "current_password_required",
                                   "currentPassword");
        }
        if (secure_strcmp(currentPassword, webui_settings()->getAdminPassword()) != 0) {
            cJSON_Delete(root);
            return send_json_error(req, "403 Forbidden", "current_password_incorrect",
                                   "currentPassword");
        }
    }
    if (hostname && !validateHostname(hostname)) {
        cJSON_Delete(root);
        return send_json_error(req, "400 Bad Request", "invalid_hostname", "hostname");
    }
    if (hostname && !useDHCP) {
        const bool local_ok =
            parse_ipv4_json(cJSON_GetObjectItem(root, "localIP"), false, &localIP);
        const bool netmask_ok =
            parse_ipv4_json(cJSON_GetObjectItem(root, "netmask"), false, &netmask);
        const bool gateway_ok =
            parse_ipv4_json(cJSON_GetObjectItem(root, "gateway"), false, &gateway);
        if (!local_ok || localIP.addr == IPADDR_ANY || !validateIPAddress(localIP) || !netmask_ok ||
            netmask.addr == IPADDR_ANY || !validateNetmask(netmask) || !gateway_ok ||
            !validateIPAddress(gateway)) {
            cJSON_Delete(root);
            return send_json_error(req, "400 Bad Request", "invalid_network", "network");
        }
    }
    if (hostname) {
        const bool dns1_ok = parse_ipv4_json(cJSON_GetObjectItem(root, "dns1"), true, &dns1);
        const bool dns2_ok = parse_ipv4_json(cJSON_GetObjectItem(root, "dns2"), true, &dns2);
        if (!dns1_ok || !dns2_ok || (dns1.addr != IPADDR_ANY && !validateIPAddress(dns1)) ||
            (dns2.addr != IPADDR_ANY && !validateIPAddress(dns2))) {
            cJSON_Delete(root);
            return send_json_error(req, "400 Bad Request", "invalid_network", "dns");
        }
    }
    if (timesource < TIMESOURCE_NTP || timesource > TIMESOURCE_GPS ||
        !validateDcfOffset(dcfOffset) || !validateGpsBaudrate(gpsBaudrate) ||
        !validateLEDBrightness(ledBrightness) || (ntpServer && !validateNtpServer(ntpServer)) ||
        (ccuIP && ccuIP[0] != '\0' && !validateCcuAddress(ccuIP))) {
        cJSON_Delete(root);
        return send_json_error(req, "400 Bad Request", "invalid_backup", "settings");
    }

    const bool has_ipv6 =
        cJSON_GetObjectItem(root, "enableIPv6") || cJSON_GetObjectItem(root, "ipv6Mode") ||
        cJSON_GetObjectItem(root, "ipv6Address") || cJSON_GetObjectItem(root, "ipv6PrefixLength") ||
        cJSON_GetObjectItem(root, "ipv6Gateway") || cJSON_GetObjectItem(root, "ipv6Dns1") ||
        cJSON_GetObjectItem(root, "ipv6Dns2");
    if (has_ipv6) {
        const bool effective_enabled = cJSON_GetObjectItem(root, "enableIPv6")
                                           ? enableIPv6
                                           : webui_settings()->getEnableIPv6();
        const char *effective_mode   = ipv6Mode ? ipv6Mode : webui_settings()->getIPv6Mode();
        const char *effective_address =
            ipv6Address ? ipv6Address : webui_settings()->getIPv6Address();
        if (!effective_mode ||
            (strcmp(effective_mode, "auto") != 0 && strcmp(effective_mode, "static") != 0 &&
            ...
            cJSON_Delete(root);
            return send_json_error(req, "400 Bad Request", "invalid_ipv6", "ipv6");
        }
        const char *optional_ipv6[] = {ipv6Gateway, ipv6Dns1, ipv6Dns2};
    ...
    if (ledPrograms && !cJSON_IsObject(ledPrograms)) {
        cJSON_Delete(root);
        return send_json_error(req, "400 Bad Request", "invalid_backup", "ledPrograms");
    }
    if (ledPrograms) {
        for (const char *key : LED_PROGRAM_KEYS) {
            cJSON *item = cJSON_GetObjectItem(ledPrograms, key);
            if (item && (!cJSON_IsNumber(item) || item->valuedouble != item->valueint ||
    ...
    std::unique_ptr<settings_snapshot_t> previous_settings(new (std::nothrow)
                                                               settings_snapshot_t{});
    std::unique_ptr<monitoring_config_t> previous_monitoring;
    if (!previous_settings) {
        cJSON_Delete(root);
        return send_json_error(req, "500 Internal Server Error", "restore_snapshot_allocation",
                               "settings");
    }
    webui_settings()->snapshot(previous_settings.get());
    if (restored_monitoring) {
        previous_monitoring.reset(new (std::nothrow) monitoring_config_t{});
        if (!previous_monitoring || monitoring_get_config(previous_monitoring.get()) != ESP_OK) {
            cJSON_Delete(root);
            return send_json_error(req, "500 Internal Server Error", "restore_snapshot_allocation",
                                   "monitoring");
        }
    ...
    if (!restore_storage) {
        cJSON_Delete(root);
        return send_json_error(req, "503 Service Unavailable", "restore_storage_busy", "backup");
    }

    if (restored_monitoring &&
        monitoring_validate_config_storage(restored_monitoring.get()) != ESP_OK) {
        cJSON_Delete(root);
        return send_json_error(req, "507 Insufficient Storage", "restore_monitoring_capacity",
                               "monitoring");
    }
    ...
    if (has_restored_theme) {
        if (theme_api_get_config(previous_theme_scheme, sizeof(previous_theme_scheme),
                                 previous_theme_color, sizeof(previous_theme_color)) != ESP_OK) {
            cJSON_Delete(root);
    ...
    if (admin_password_change_requested) {
        ...
        if (token_result != ESP_OK) {
            ...
            cJSON_Delete(root);
            return send_json_error(req, "500 Internal Server Error",
                                   "restore_token_rotation_failed", "adminPassword");
        }
    }
    ...
    if (admin_password_change_requested) {
        if (has_password_changed_flag) {
            (void)webui_settings()->restoreAdminPassword(adminPassword,
                                                         cJSON_IsTrue(passwordChangedItem));
        } else {
            (void)webui_settings()->setAdminPassword(adminPassword);
        }
    }
    if (hostname) {
        (void)webui_settings()->setNetworkSettings(hostname, useDHCP, localIP, netmask, gateway,
                                                   dns1, dns2);
    }
    ...
    if (ledPrograms) {
        for (int index = 0; index < 7; ++index) {
            cJSON *item = cJSON_GetObjectItem(ledPrograms, LED_PROGRAM_KEYS[index]);
            if (item) {
    ...
    if (has_ipv6) {
        webui_settings()->setIPv6Settings(
            cJSON_GetObjectItem(root, "enableIPv6") ? enableIPv6
                                                    : webui_settings()->getEnableIPv6(),
            ipv6Mode ? ipv6Mode : webui_settings()->getIPv6Mode(),
            ipv6Address ? ipv6Address : webui_settings()->getIPv6Address(),
            cJSON_GetObjectItem(root, "ipv6PrefixLength") ? ipv6PrefixLength
                                                          : webui_settings()->getIPv6PrefixLength(),
    ...
    const esp_err_t settings_capacity_result = webui_settings()->validateStorageCapacity();
    if (settings_capacity_result != ESP_OK) {
        ...
        if (settings_capacity_result == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
            return send_json_error(req, "507 Insufficient Storage", "restore_settings_capacity",
                                   "settings");
        }
        return send_json_error(req, "500 Internal Server Error",
                               "restore_settings_validation_failed", "settings");
    }
    ...
    const esp_err_t restore_begin_result = Settings::beginRestoreTransaction();
    if (restore_begin_result != ESP_OK) {
        webui_settings()->restoreSnapshot(previous_settings.get());
        cJSON_Delete(root);
        if (restore_begin_result == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
            return send_json_error(req, "507 Insufficient Storage", "restore_storage_capacity",
                                   "backup");
        }
        return send_json_error(req, "500 Internal Server Error", "restore_transaction_begin_failed",
                               "backup");
    }

    auto finish_rollback_transaction = [](const char *context, esp_err_t settings_rollback,
                                          esp_err_t monitoring_rollback,
                                          esp_err_t theme_rollback) -> bool {
        if (settings_rollback != ESP_OK || monitoring_rollback != ESP_OK ||
            theme_rollback != ESP_OK) {
            ESP_LOGE(TAG, "%s; restore marker retained: settings=%s monitoring=%s theme=%s",
                     context, esp_err_to_name(settings_rollback),
                     esp_err_to_name(monitoring_rollback), esp_err_to_name(theme_rollback));
            return false;
        }
        const esp_err_t finish_result = Settings::finishRestoreTransaction();
        if (finish_result != ESP_OK) {
            ESP_LOGE(TAG, "%s; could not clear restore marker: %s", context,
                     esp_err_to_name(finish_result));
            return false;
        }
        return true;
    };
    ...
    if (settings_save_result != ESP_OK) {
        ...
        const bool rollback_complete    = finish_rollback_transaction(
            "Could not roll back Settings after failed restore", rollback_result, ESP_OK, ESP_OK);
        cJSON_Delete(root);
        if (!rollback_complete) {
            return send_json_error(req, "500 Internal Server Error", "restore_rollback_failed",
                                   "backup");
        }
        if (settings_save_result == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
            return send_json_error(req, "507 Insufficient Storage", "restore_settings_failed",
                                   "settings");
        }
        return send_json_error(req, "500 Internal Server Error", "restore_settings_failed",
                               "settings");
    }
    ...
    if (has_restored_theme) {
        const esp_err_t theme_result =
            theme_api_set_config(restored_theme_scheme, restored_theme_color);
        if (theme_result != ESP_OK) {
            const esp_err_t theme_rollback =
                theme_api_set_config(previous_theme_scheme, previous_theme_color);
            webui_settings()->restoreSnapshot(previous_settings.get());
            const esp_err_t rollback_result = webui_settings()->save();
            const bool rollback_complete =
                finish_rollback_transaction("Could not fully roll back theme restore failure",
            ...
            if (theme_result == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
                return send_json_error(req, "507 Insufficient Storage", "restore_theme_failed",
                                       "theme");
            }
            return send_json_error(req, "500 Internal Server Error", "restore_theme_failed",
                                   "theme");
        }
        theme_written = true;
    }
    ...
    if (restored_monitoring) {
        ...
        if (final_monitoring_capacity != ESP_OK) {
            ...
            if (theme_written) {
                theme_rollback = theme_api_set_config(previous_theme_scheme, previous_theme_color);
            }
            const bool rollback_complete = finish_rollback_transaction(
                "Restore capacity rollback incomplete", settings_rollback, ESP_OK, theme_rollback);
            cJSON_Delete(root);
            if (!rollback_complete) {
                return send_json_error(req, "500 Internal Server Error", "restore_rollback_failed",
                                       "backup");
            ...
            if (final_monitoring_capacity == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
            ...
            return send_json_error(req, "500 Internal Server Error",
                                   "restore_monitoring_validation_failed", "monitoring");
        }
        ...
        if (monitoring_result != ESP_OK) {
            ...
            const bool rollback_complete =
                finish_rollback_transaction("Restore rollback incomplete", settings_rollback,
            ...
            return send_json_error(req, "500 Internal Server Error", "restore_monitoring_failed",
                                   "monitoring");
        }
    }

    const esp_err_t restore_finish_result = Settings::finishRestoreTransaction();
    if (restore_finish_result != ESP_OK) {
        webui_settings()->restoreSnapshot(previous_settings.get());
        const esp_err_t settings_rollback = webui_settings()->save();
        esp_err_t monitoring_rollback     = ESP_OK;
        if (restored_monitoring) {
            monitoring_rollback = monitoring_save_config_for_restore(previous_monitoring.get());
        }
        esp_err_t theme_rollback = ESP_OK;
        if (theme_written) {
            theme_rollback = theme_api_set_config(previous_theme_scheme, previous_theme_color);
        ...
        const bool rollback_complete =
            finish_rollback_transaction("Restore marker finalization failed", settings_rollback,
                                        monitoring_rollback, theme_rollback);
        cJSON_Delete(root);
        if (!rollback_complete) {
            return send_json_error(req, "500 Internal Server Error", "restore_rollback_failed",
...
httpd_uri_t post_restore_handler = {.uri      = "/api/restore",
                                    .method   = HTTP_POST,
