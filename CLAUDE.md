...
> at the GitHub releases page to discover new versions. The dead
> `scripts/update_archive.py` helper and the `main/generated/archive.json.gz`
> artifact have been removed from the tree, together with the release-workflow
> steps that regenerated them. The root `archive.json` / `webui-archive.json`
> files are still live — the release workflows publish them to GitHub Pages.
