  ...
  <BCard>
    ...
    <BForm>
      ...
      <template v-if="snmpConfig.enabled">
        <BFormGroup label-cols-sm="4" :label="t('monitoring.snmp.port')">
          <BFormInput
            v-model.number="snmpConfig.port"
            type="number"
            min="1"
            max="65535"
          />
    ...
    <BForm>
      ...
      <template v-if="checkmkConfig.enabled">
        <BFormGroup label-cols-sm="4" :label="t('monitoring.checkmk.port')">
          <BFormInput
            v-model.number="checkmkConfig.port"
            type="number"
            min="1"
            max="65535"
          />
        ...
        <BFormGroup
          label-cols-sm="4"
          :label="t('monitoring.checkmk.allowedHosts')"
        >
    ...
    <BForm>
      ...
      <template v-if="mqttConfig.enabled">
        ...
        <BFormGroup
          label-cols-sm="4"
          :label="t('monitoring.mqtt.haDiscoveryEnabled')"
        >
        ...
        <template v-if="mqttConfig.haDiscoveryEnabled">
          <BFormGroup
            label-cols-sm="4"
            :label="t('monitoring.mqtt.haDiscoveryPrefix')"
          >
            <BFormInput v-model="mqttConfig.haDiscoveryPrefix" />
            <BFormText>{{
              t("monitoring.mqtt.haDiscoveryPrefixHelp")
            }}</BFormText>
          </BFormGroup>
        </template>
      </template>
