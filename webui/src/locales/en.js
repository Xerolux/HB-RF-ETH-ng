export default {
  // Common
  common: {
    enabled: 'Enabled',
    disabled: 'Disabled',
    save: 'Save',
    cancel: 'Cancel',
    close: 'Close',
    loading: 'Loading...',
    changing: 'Changing...',
    error: 'Error',
    success: 'Success',
    yes: 'Yes',
    no: 'No',
    ok: 'OK',
    unknownError: 'Unknown error',
    copyFailed: 'Copy failed',
    clear: 'Clear',
    network: {
      serverErrorTitle: 'Server Error',
      serverError: 'Server error: {status}',
      timeoutTitle: 'Request Timeout',
      timeout: 'The request took too long. Please try again.',
      connectionErrorTitle: 'Connection Error',
      connectionError: 'Unable to connect to the device. Check your network.'
    }
  },

  // App-level chrome (footer copyright, sponsor button)
  app: {
    footerCopyright: 'HB-RF-ETH-ng {version} © 2025-2026 Xerolux',
    sponsor: 'Sponsor',
    followOnX: 'Follow on X'
  },

  // Header Navigation
  nav: {
    home: 'Home',
    settings: 'Settings',
    firmware: 'Firmware',
    updates: 'Updates',
    documentation: 'Documentation',
    monitoring: 'Monitoring',
    diagnostics: 'Radio & relay',
    systemlog: 'System Log',
    about: 'About',
    login: 'Login',
    logout: 'Logout',
    toggleTheme: 'Toggle theme',
    language: 'Language',
    mainMenu: 'Main menu',
    groupOverview: 'Overview',
    groupSystem: 'System',
    groupInfo: 'Info'
  },

  // Login Page
  login: {
    title: 'Please log in',
    subtitle: 'Please enter username and password to continue',
    username: 'Username',
    usernamePlaceholder: 'Username',
    password: 'Password',
    passwordPlaceholder: 'Password',
    login: 'Login',
    loginFailed: 'Login failed',
    invalidCredentials: 'Invalid credentials',
    loginError: 'Login was not successful.',
    usernameRequired: 'Username is required',
    passwordRequired: 'Password is required',
    loggingIn: 'Logging in...',
    forgotPassword: 'Forgot password?',
    passwordResetTitle: 'Reset password',
    passwordResetPressButton: 'Briefly press the HM button on the board now (GPIO34 / HM_BTN) to confirm physical access.',
    passwordResetExpires: 'Time window: {seconds} seconds',
    passwordResetConfirmed: 'Board button detected',
    passwordResetEnterNew: 'Please set a new administrator password now.',
    newPasswordPlaceholder: 'New password',
    confirmPasswordPlaceholder: 'Repeat new password',
    passwordResetSave: 'Save password',
    passwordResetPolicy: 'Use 8 to 32 characters with uppercase, lowercase, and a number.',
    passwordResetMismatch: 'The passwords do not match.',
    passwordResetExpired: 'The time window expired. Please start again.',
    passwordResetError: 'Password reset was not successful.',
    footerCopyright: '© Xerolux 2026',
    githubLink: 'GitHub'
  },

  // Settings Page
  settings: {
    ledPatternOff: 'Off',
    ledPatternOn: 'On',
    ledPatternBlink: 'Blink',
    ledPatternBlinkInv: 'Blink inverted',
    ledPatternFastBlink: 'Fast blink',
    ledPatternSlowBlink: 'Slow blink',
    ledPatternBlink2x: '2x blink',
    ledPatternBlink3x: '3x blink',
    ledPatternBreathing: 'Breathing',
    ledPatternHeartbeat: 'Heartbeat',
    ledPatternStrobe: 'Strobe',
    ledProgramIdle: 'Idle',
    ledProgramCcuDisconnected: 'CCU disconnected',
    ledProgramCcuConnected: 'CCU connected',
    ledProgramUpdateAvailable: 'Update available',
    ledProgramError: 'Error',
    ledProgramBooting: 'Booting',
    ledProgramUpdateInProgress: 'Update in progress',
    unsavedLeave: 'You have unsaved changes. Leave this page anyway?',
    unsavedChanges: 'Unsaved changes',
    allSaved: 'Saved',
    selected: 'Selected: {name}',
    discardConfirm: 'Discard your unsaved changes and restore the last loaded values?',
    discard: 'Discard',
    restartError: 'Error restarting device',
    title: 'Settings',
    ccuSettings: 'CCU Connection',
    ccuIpAddress: 'CCU IP Address',
    ccuIpHint: 'Please enter the IP address of your CCU to prevent connection blocking. System will restart after saving.',
    tabGeneral: 'General',
    tabNetwork: 'Network',
    tabTime: 'Time',
    tabBackup: 'Backup & Reset',
    tabDesign: 'Design',
    tabExperimental: 'Experimental',
    advancedTitle: 'Advanced',
    showExperimental: 'Show experimental features',
    showExperimentalHint: 'Reveals experimental menu entries, tabs and settings. Stored experimental values are preserved when hidden again.',
    showExperimentalWarning: 'Experimental features are unfinished and may change or not work as expected.',
    showExperimentalOn: 'Experimental features shown',
    showExperimentalOff: 'Experimental features hidden',
    changePassword: 'Change Password',
    repeatPassword: 'Repeat Password',
    hostname: 'Hostname',

    // Security Settings
    security: 'Security',
    adminUsername: 'Administrator username',
    adminUsernameHint: 'Defaults to admin. Change it here for password managers or multi-device setups.',
    changePasswordHint: 'Change your administrator password for secure access',
    changePasswordBtn: 'Change Password',

    // Network Settings
    networkSettings: 'Network Settings',
    dhcp: 'DHCP',
    ipAddress: 'IP Address',
    netmask: 'Netmask',
    gateway: 'Gateway',
    dns1: 'Primary DNS',
    dns2: 'Secondary DNS',

    // IPv6 Settings
    ipv6Settings: 'IPv6 Settings',
    enableIPv6: 'Enable IPv6',
    ipv6Mode: 'IPv6 Mode',
    ipv6Auto: 'Automatic (SLAAC)',
    ipv6Static: 'Static',
    ipv6Address: 'IPv6 Address',
    ipv6PrefixLength: 'Prefix Length',
    ipv6Gateway: 'IPv6 Gateway',
    ipv6Dns1: 'Primary IPv6 DNS',
    ipv6Dns2: 'Secondary IPv6 DNS',

    // Time Settings
    timeSettings: 'Time Settings',
    timesource: 'Time Source',
    ntp: 'NTP',
    dcf: 'DCF',
    gps: 'GPS',
    ntpServer: 'NTP Server',
    dcfOffset: 'DCF Offset',
    gpsBaudrate: 'GPS Baudrate',

    // System Settings
    systemSettings: 'System Settings',
    ledBrightness: 'LED Brightness',
    ledPrograms: 'LED Programs',
    ledProgramsHelp: 'Customize the LED behavior for different system states.',
    language: 'Language',

    // Messages
    saveSuccess: 'Settings were successfully saved.',
    saveError: 'An error occurred while saving the settings.',
    restartTitle: 'Restart Required',
    restartMessage: 'Settings have been saved. Do you want to restart the device now for changes to take effect?',
    restartNow: 'Restart Now',
    restartLater: 'Restart Later',

    // Backup & Restore
    backupRestore: 'Backup & Restore',
    backupInfo: 'Complete, unencrypted backup of all user settings, including credentials, monitoring, certificates, network, time, LEDs and design.',
    backupSecurityTitle: 'Warning: plaintext credentials',
    backupSecurityText: 'Passwords, tokens, certificates and private keys are stored unencrypted in the JSON file. Treat the backup like a password and never publish it.',
    backupTemplateInfo: 'The JSON file can be edited in a text editor and imported on additional devices. First change at least the hostname, static IP, administrator password and device-specific MQTT values to prevent conflicts.',
    backupFileWarning: 'This unencrypted backup contains passwords, tokens, certificates and private keys in plaintext. Store it like a password and never publish it.',
    backupFileTemplateWarning: 'Before importing on another device, review and adjust the hostname, static IP, administrator password and device-specific MQTT values.',
    restoreInfo: 'Restores the complete backup, including credentials, monitoring, certificates and design. The system will restart afterwards.',
    downloadBackup: 'Download Backup',
    restore: 'Restore',
    restoreBtn: 'Restore',
    download: 'Download',
    noFileChosen: 'No file chosen',
    browse: 'Browse',
    restoreConfirm: 'Are you sure? Current settings will be overwritten and the system will restart.',
    systemActions: 'System actions',
    openRecovery: 'Open recovery page',
    openRecoveryHint: 'Open the independent recovery and diagnostics page.',
    experimentalTitle: 'Experimental',
    experimentalEmptyTitle: 'Nothing here yet',
    experimentalEmptyText: 'No experimental features are currently available. The New Design and Restart Sync are standard for all devices.',
    experimentalWarningTitle: 'Work in progress',
    experimentalWarningText: 'These features are intended for testing. There is no guarantee for function or appearance.',
    experimentalDesign: 'Test design',
    experimentalDesignHint: 'Enables the new experimental layout. The choice is stored on the device and survives restarts.',
    flashPause: 'Restart Sync',
    flashPauseHint: 'When a restart is triggered by a firmware update or settings change, the Ethernet link is held down for ~35 seconds before the device reboots. This lets the CCU watchdog (30 s timeout) detect the disconnect and perform a clean CCU reboot, preventing stale connections.',
    restoreSuccess: 'Settings successfully restored. System restarting...',
    restoreError: 'Error restoring settings',
    backupError: 'Error downloading backup',
    validation: {
      invalidUsername: 'The username may contain only letters, numbers, dots, dashes, and underscores.',
      invalidHostname: 'Invalid hostname.',
      invalidIpv4: 'Invalid IPv4 address.',
      invalidIpv4OrIpv6: 'Invalid IPv4 or IPv6 address',
      invalidNetmask: 'Invalid subnet mask. Its set bits must be contiguous.',
      invalidNetwork: 'The network settings are invalid.',
      invalidNtpServer: 'Invalid NTP server or port.',
      invalidIpv6: 'Invalid IPv6 address',
      invalidIpv6Prefix: 'The IPv6 prefix must be between 1 and 128.',
      invalidIpv6Mode: 'Invalid IPv6 mode.',
      invalidTimeSource: 'Invalid time source.',
      invalidDcfOffset: 'Invalid DCF offset.',
      invalidGpsBaudrate: 'Invalid GPS baud rate.',
      invalidLedBrightness: 'LED brightness must be between 0 and 100 percent.',
      minPrefix: 'Min 1',
      maxPrefix: 'Max 128',
      fixErrors: 'Please correct the highlighted fields.'
    },
    ping: {
      title: 'Network Diagnostics',
      target: 'Ping Target (IP or Hostname)',
      placeholder: '192.168.1.1',
      button: 'Ping',
      success: 'Ping successful. Latency: {latency} ms',
      failure: 'Ping failed.',
      invalid_target: 'The ping target is invalid.',
      dns_failed: 'The hostname could not be resolved.',
      timeout: 'The target did not respond before the timeout.',
      unauthorized: 'Your session has expired. Please sign in again.',
      internal: 'The network diagnostic could not be run.'
    }
  },

  // System Info
  diagnostics: {
    eyebrow: 'Diagnostics',
    pageTitle: 'Radio and relay statistics',
    pageDescription: 'Figures from the raw-UART relay between the CCU and the radio module. They show whether datagrams were delayed or dropped — and therefore whether a reported device fault sits in the device or in the relay.',
    sessionActive: 'CCU connected',
    sessionIdle: 'no CCU session',
    throughputKicker: 'Throughput',
    throughputHeading: 'Frames relayed',
    throughputHelp: 'The denominator for everything below: a drop count can only be judged against the traffic it occurred in.',
    rxFrames: 'Received from the CCU',
    txFrames: 'Sent to the CCU',
    keepalives: 'Keepalives',
    drops: 'Dropped',
    latencyKicker: 'Latency',
    latencyHeading: 'Queue and delays',
    latencyHelp: 'The queue is the last buffer before a datagram is dropped outright. If it fills up, or frames wait a long time, switching commands arrive late.',
    queueWaitMax: 'Longest wait',
    queueDepthMax: 'Peak occupancy',
    waitOver10ms: 'Delayed over 10 ms',
    waitOver100ms: 'Delayed over 100 ms',
    waitOver1s: 'Delayed over 1 s',
    lastRx: 'Last packet',
    highWaterNote: 'Peak values persist until reboot and never decay. Reset them to watch a fresh window — the frame totals are kept.',
    resetButton: 'Reset peak values',
    reloadButton: 'Reload',
    resetDone: 'Peak values reset',
    resetFailed: 'Reset failed',
    loadError: 'The statistics could not be loaded.',
    unsupported: 'This firmware does not report relay statistics yet, so the figures below say nothing. Update the firmware first.',
    txFailed: 'Send failures to CCU',
    uartKicker: 'Radio module',
    uartHeading: 'Radio module link (UART)',
    uartHelp: 'The numbers above measure the path to the CCU. These measure the path to the radio module — the only path on which received radio frames can be discarded without any CCU-side counter moving.',
    uartUnsupported: 'This firmware does not report radio module counters yet. Please update to a newer version.',
    uartOverflows: 'Buffer overflows',
    uartLostBytes: 'Discarded bytes',
    uartLineErrors: 'Line errors',
    uartTxErrors: 'Send errors',
    uartBacklogMax: 'Peak backlog',
    uartRxFrames: 'Frames from module',
    uartTxFrames: 'Frames to module',
    uartOversize: 'Oversized events',
    uartBreaks: 'Break conditions',
    uartParityErrors: 'Parity errors',
    uartFrameErrors: 'Framing errors',
    uartResetLineEvents: 'After module reset',
    uartModuleResetsCcu: 'Module resets by CCU',
    uartModuleResets: 'Module resets total',
    uartNote: 'Buffer overflows, discarded bytes, line errors or send errors other than zero mean radio traffic was lost inside this firmware. The CCU then repeats the transmission, which raises the duty cycle and eventually makes a device appear faulty. If you see it, please post a screenshot in issue #447.',
    uartResetNote: 'Line errors shortly after a module reset (breaks within 10 s, parity and framing errors within 0.5 s) are counted under “After module reset” and are not a finding: the module is coming back up and no frame is in flight. What matters is the number of module resets requested by the CCU. A CCU start issues three to five; if the figure keeps climbing in steady state, the CCU keeps re-opening the link, and every reset takes the module off the air for seconds — the cause is then on the CCU, not on this board. Every reset is in the system log with timestamp and requester. The “Reset peak values” button also clears the counters on this card; the frame totals are kept.',
    interpretHint: 'How to read this: if “Dropped” and the delay counters are zero and peak occupancy is low, the relay is working cleanly and a reported device fault has another cause. If occupancy approaches capacity, or delays above 100 ms appear, the relay path is implicated.',
  },
  sysinfo: {
    updatesRecovery: 'Updates and recovery',
    liveStreamExport: 'Live stream and export',
    networkTimeSetup: 'Network and time setup',
    copied: '{label} copied',
    title: 'System information',
    goodMorning: 'Good Morning',
    goodAfternoon: 'Good Afternoon',
    goodEvening: 'Good Evening',
    serial: 'Serial number',
    hostname: 'Hostname',
    boardRevision: 'Board revision',
    uptime: 'Uptime',
    // Value is the cause of the last reboot, not a timestamp — name the cause
    // explicitly and keep it separate from the uptime row (Korrekturauftrag §1.5).
    resetReason: 'Restart reason',
    cpuUsage: 'CPU usage',
    memoryUsage: 'Memory usage',
    ethernetStatus: 'Ethernet connection',
    rawUartRemoteAddress: 'Connected CCU',
    localIP: 'IPv4 Address',
    netmask: 'Subnet Mask',
    gateway: 'Gateway',
    dns: 'DNS Server',
    ipv6Addresses: 'IPv6 Addresses',
    radioModuleType: 'Radio module type',
    radioModuleSerial: 'Serial number',
    radioModuleFirmware: 'Firmware version',
    radioModuleBidCosRadioMAC: 'Radio address (BidCoS)',
    radioModuleHmIPRadioMAC: 'Radio address (HmIP)',
    radioModuleSGTIN: 'SGTIN',
    version: 'Version',
    memory: 'Memory Usage',
    cpu: 'CPU Usage',
    temperature: 'Temperature',
    voltage: 'Supply Voltage',
    ethernet: 'Ethernet',
    connected: 'Connected',
    disconnected: 'Disconnected',
    speed: 'Speed',
    duplex: 'Duplex',
    radioModule: 'Radio Module',
    moduleType: 'Module Type',
    firmwareVersion: 'Firmware Version',
    bidcosMAC: 'BidCoS Radio MAC',
    hmipMAC: 'HmIP Radio MAC',
    system: 'System',
    network: 'Network',
    down: 'Down',
    mbits: 'Mbit/s',
    online: 'Online',
    offline: 'Offline',
    dashboardTitle: 'System Status',
    monitoringSummary: 'MQTT, CheckMK, Prometheus, Syslog & Notifications',
    lan: 'LAN',
    boardSenseHint: 'Raw ADC reading from the board-revision sense pin. Compare against the four nominal divider voltages to identify the board revision: Rev 1.10/1.11 PUB ≈ 550 mV, Rev 1.8 SK ≈ 1650 mV, Rev 1.8 PUB ≈ 2750 mV, Rev 1.10/1.11 SK ≈ 3050 mV.'
  },

  // Updates page (merged Firmware + WebUI entry, Korrekturauftrag §6)
  updates: {
    firmware: 'Device firmware',
    firmwareDescription: 'ESP32, network and radio',
    webui: 'WebUI',
    webuiDescription: 'Browser-based controls'
  },

  // WebUI Update page (separate WebUI tab)
  webuiUpdate: {
    pageTitle: 'Update the web interface',
    pageSubtitle: 'Firmware and WebUI have separate versions. Installation is done exclusively via a manually selected 320 KB WWW image.',
    webuiChip: 'WebUI',
    firmwareChip: 'Firmware',
    separateActive: 'Separate WebUI active',
    embeddedFallback: 'Embedded fallback active',
    compatibilityWarningTitle: 'WebUI compatibility problem',
    compatibilityWarningApi: 'Installed WebUI v{version} requires API {required}; this firmware supports API {supported}. The embedded WebUI is active for safety.',
    compatibilityWarningFirmware: 'Installed WebUI v{version} requires firmware {minimum} or newer; {current} is installed. The embedded WebUI is active for safety.',
    compatibilityWarningInvalid: 'The external WebUI is incomplete or has no valid compatibility contract. The embedded WebUI is active for safety.',
    compatibilityWarningAction: 'Repair WebUI',
    installedLabel: 'Installed WebUI version',
    separateSource: 'From the separate WWW partition',
    embeddedSource: 'Embedded in the firmware',
    storageLabel: 'WWW storage',
    availableKicker: 'Available WebUI',
    releaseLink: 'Release on GitHub',
    onlineHelp: 'The device fetches a small version file and compares it with the installed WebUI. Nothing is downloaded or installed.',
    upToDate: 'The WebUI is current — {version} is the latest published version.',
    installHint: 'To install, download the webui_*.bin from the release and upload it below.',
    manualKicker: 'Manual installation',
    uploadHeading: 'Upload WWW image',
    uploadHelp: 'Use only webui_*.bin or spiffs.bin with exactly {size} here.',
    chooseFile: 'Choose WebUI BIN',
    uploadInProgress: 'WebUI is being transferred …',
    uploadExplainer: 'The device checks the partition size, internal manifest and optionally SHA-256. If the image is invalid or incomplete, the embedded New Design remains active as a fallback.',
    installingButton: 'WebUI is being installed …',
    installButton: 'Install WebUI',
    clearSelectionButton: 'Clear selection',
    unknownVersion: 'unknown',
    statusLoadError: 'Device status could not be loaded. Please reload the page.',
    fileInvalidExtension: 'Invalid file: a WebUI image with the .bin extension is required.',
    fileIsFirmware: 'Wrong file: this is a firmware file. Please install it under System → Firmware.',
    fileStatusMissing: 'Device status unavailable. Please reload the page and try again.',
    fileWrongSize: 'Wrong image size: exactly {expected} ({bytes} bytes) are expected.',
    installSuccessTitle: 'WebUI installed',
    installSuccessMessage: 'Active WebUI version: {version}',
    uploadFailedTitle: 'WebUI upload failed',
    uploadFailedMessage: 'WebUI upload failed.'
  },

  // Firmware Update
  firmware: {
    pageTitle: 'Update device firmware',
    pageDescription: 'The device firmware controls the ESP32, network and radio connection. It is updated independently of the WebUI.',
    installedLabel: 'Installed',
    availableKicker: 'Available device firmware',
    manualKicker: 'Manual installation',
    manualHeading: 'Install firmware manually',
    manualHelp: 'Use only firmware_*.bin here. WebUI files belong under System → WebUI.',
    selectFirmwareBin: 'Select a firmware BIN or drop it here',
    writeWarning: 'Do not interrupt the power supply or network while writing. After a successful upload, the device restarts with permanent Restart Sync enabled.',
    restartSyncHint: 'Restart the device with active Restart Sync',
    factoryResetActionHint: 'Delete all stored settings',
    fileInvalidExtension: 'Invalid file: a firmware file with the .bin extension is required.',
    fileIsWebui: 'Wrong file: this is a WebUI/WWW image. Please install it under System → WebUI.',
    fileTooSmall: 'The selected file is too small to be valid firmware.',
    fileInvalidMagic: 'Invalid firmware image: the ESP32 file header 0xE9 is missing. The file was not accepted.',
    fileReadError: 'The selected file could not be checked.',
    uploadCompleteTitle: 'Firmware transferred',
    uploadCompleteMessage: 'The device is restarting now.',
    uploadFailedTitle: 'Firmware upload failed',
    uploadFailedMessage: 'The firmware upload failed.',
    restarting: 'Restarting...',
    restartingText: 'Device is restarting. Page will reload automatically.',
    factoryResetTitle: 'Factory reset',
    factoryResetMessage: 'Do you really want to reset all settings to factory defaults?',
    factoryResetWarning: 'All user data will be erased, including credentials, network, monitoring, certificates, logs, theme, and accent color. The device will restart automatically afterwards.',
    factoryResetConfirm: 'Reset to factory defaults',
    factoryResetError: 'Factory reset failed.',
    factoryResetChallengeHelp: 'Enter the following 8-character security code exactly. It is case-sensitive; copying and pasting are disabled.',
    factoryResetChallengeLabel: 'Security code',
    factoryResetChallengeDisplayLabel: 'Security code: {code}',
    factoryResetChallengeInputLabel: 'Confirm security code',
    factoryResetChallengePlaceholder: 'Enter the code exactly',
    factoryResetChallengeMismatch: 'The entered code does not match exactly.',
    factoryResetChallengeReady: 'Code confirmed. Factory reset is now unlocked.',
    upload: 'Install Firmware',
    restart: 'Restart',
    uploading: 'Uploading...',
    restartConfirm: 'Do you really want to restart the system?',
    onlineKicker: 'Update search',
    onlineHeading: 'Search for updates online',
    onlineHelp: 'The device fetches a small version file and compares it with what is installed. Nothing is downloaded or installed.',
    onlineManualNote: 'The search runs only when you press the button — the device never searches on its own in the background.',
    checkNow: 'Search for updates now',
    checking: 'Searching …',
    channelLabel: 'Channel',
    channelStable: 'Stable',
    channelBeta: 'Beta',
    neverChecked: 'No search has been run yet.',
    lastCheck: 'Last search',
    upToDate: 'Everything is current — firmware {fw} and WebUI {ui} are the latest versions.',
    firmwareAvailable: 'New firmware available: {version}',
    webuiAvailable: 'New WebUI available: {version}',
    installHint: 'To install, download the file from the release and upload it below.',
    releaseNotes: 'View release notes',
    checkSkipped: 'Search skipped: {reason}',
    checkFailed: 'Search failed: {reason}',
    checkCooldown: 'Please wait a moment — a search has just been run.',
    checkCooldownRetry: 'A search just ran — try again in {seconds} seconds.',
    skipLowMemory: 'too little free memory (free: {free} KB, largest block: {largest} KB)',
    skipInstallRunning: 'an update installation is currently running',
    skipNetworkBusy: 'the network subsystem is busy',
    skipManifestBuffer: 'not enough memory for the version buffer',
    checkBusy: 'A search is already running.',
    checkUnavailable: 'The search cannot be started right now.',
    viewOnGithub: 'View on GitHub',
    restartFlashPauseHint: 'Restart Sync is active — the Ethernet link will be held down for ~{seconds} s before the device is available again after the restart, so the page reload will take a bit longer than usual.'
  },

  // Monitoring
  monitoring: {
    selfTestHint: 'Run a quick self-test to verify the current configuration.',
    selfTestTitle: 'Self-test',
    testButton: 'Test',
    checkFinished: 'Check finished',
    diagnosticFailed: 'Diagnostic request failed',
    chipLabelCheckmk: 'CheckMK',
    chipLabelMqtt: 'MQTT',
    chipLabelPrometheus: 'Prometheus',
    chipLabelSyslog: 'Syslog',
    chipLabelNotify: 'Notifications',
    title: 'Monitoring',
    description: 'Configure monitoring and notifications for the HB-RF-ETH gateway.',
    resourceWarningTitle: 'Keep resources in mind',
    resourceWarningText: '{count} monitoring services are enabled. The ESP32 can usually handle this, but concurrent TLS connections and a manual firmware upload share heap and CPU; if problems occur, disable individual services or stagger tests.',
    save: 'Save',
    saving: 'Saving...',
    saveSuccess: 'Configuration saved successfully!',
    saveError: 'Error saving configuration!',
    checkmk: {
      title: 'CheckMK Agent',
      enabled: 'Enable CheckMK',
      port: 'Port',
      portHelp: 'Default: 6556',
      allowedHosts: 'Allowed Client IPs',
      allowedHostsHelp: 'Comma-separated IP addresses (e.g. "192.168.1.10,192.168.1.20") or "*" for all'
    },
    mqtt: {
      title: 'MQTT Client',
      enabled: 'Enable MQTT',
      server: 'Server',
      serverHelp: 'MQTT Broker Hostname or IP',
      port: 'Port',
      portHelp: 'Default: 1883 (8883 for TLS/SSL)',
      user: 'User',
      userHelp: 'Optional: MQTT Username',
      password: 'Password',
      passwordHelp: 'Optional: MQTT Password',
      topicPrefix: 'Topic Prefix',
      topicPrefixHelp: 'Root segment of every MQTT topic this device publishes/subscribes. Default: hb-rf-eth-ng. Examples: “prefix/status/uptime”, “prefix/status/version”, “prefix/command/restart”. Visible in your MQTT broker (e.g. MQTT Explorer), not in the WebUI.',
      haDiscoveryEnabled: 'Home Assistant Discovery',
      haDiscoveryPrefix: 'Discovery Prefix',
      haDiscoveryPrefixHelp: 'Default: homeassistant',
      serverRequired: 'Please enter an MQTT server address when MQTT is enabled.',
      commands: {
        title: 'Command Topics',
        enableHelp: 'Allows device restart over MQTT. Factory reset and OTA are intentionally unavailable as MQTT commands.',
        token: 'Command Token',
        tokenPlaceholder: 'optional – shared secret',
        tokenHelp: 'When set, command payloads must exactly match this token. Allowed: A-Z a-z 0-9 - _ .',
        tokenPresent: '✓ Present – enter new value to replace',
        clear: 'Clear',
        aclHint: 'Note: without a broker ACL and token, any MQTT client can restart the device. Configure an ACL that grants command-topic publish rights only to authorized clients.'
      },
      tls: {
        title: 'TLS / SSL',
        enable: 'Enable TLS',
        enableHelp: 'Optional: Secure the MQTT connection via TLS',
        skipVerify: 'Disable certificate verification',
        skipVerifyAlert: 'Warning: With certificate verification disabled, man-in-the-middle attacks cannot be detected. Use only in test environments or with self-signed certificates.',
        caCerts: 'CA Certificate (PEM)',
        caCertsHelp: 'Leave empty = use the built-in Mozilla root CA bundle of the firmware (suitable for public cloud brokers)',
        certfile: 'Client Certificate (mTLS, PEM)',
        certfileHelp: 'Optional: mTLS client certificate',
        keyfile: 'Client Key (mTLS, PEM)',
        keyfileHelp: 'Optional: mTLS client key',
        certPresent: '✓ Present – enter new value to replace',
        clear: 'Clear',
        loadFromFile: 'Load from file',
        fileTooLarge: 'File too large (max. 16 KB for PEM content).',
        fileReadError: 'Could not read file.',
        pemValid: '✓ PEM format recognized ({count} block | {count} blocks).',
        pemTooLarge: 'Content too large (max. 8 KB per field).',
        keySingleBlock: 'Key file must contain exactly one PRIVATE KEY block.',
        encryptedKeyError: 'Encrypted private keys are not supported. Please export without password: openssl pkcs8 -topk8 -nocrypt -in key.pem -out key-plain.pem',
        invalidPemFormat: 'No valid PEM content detected (BEGIN/END markers missing or mismatched).',
        unexpectedPemLabel: 'Unexpected PEM block type for this field.',
        binaryFormatError: 'Binary format (DER) detected – please convert to PEM first: openssl x509 -inform DER -in cert.der -out cert.pem',
        pkcs12Error: 'PKCS#12 container (.p12/.pfx) is not supported. Please extract: openssl pkcs12 -in bundle.p12 -nodes -out cert.pem',
        invalidBase64: 'PEM body contains invalid characters.',
        mtlsRequiresBoth: 'For mTLS both certificate and key must be provided.'
      }
    },
    enable: 'Enable',
    allowedHosts: 'Allowed Hosts',
    prometheus: {
      title: 'Prometheus Exporter',
      port: 'Port',
      portHelp: 'Default: 9100',
      allowedHosts: 'Allowed Client IPs',
      allowedHostsHelp: 'Comma-separated IP addresses or "*" for all'
    },
    syslog: {
      title: 'Syslog Forwarding',
      server: 'Server',
      serverHelp: 'Hostname or IP of the syslog server',
      port: 'Port',
      portHelp: 'Default: 514',
      transport: 'Transport',
      transportUdp: 'UDP',
      transportTcp: 'TCP',
      transportTls: 'TLS',
      minSeverity: 'Minimum Severity',
      minSeverityHelp: 'Only forward messages at or above this severity',
      severityEmerg: 'EMERG',
      severityAlert: 'ALERT',
      severityCrit: 'CRIT',
      severityErr: 'ERR',
      severityWarning: 'WARNING',
      severityNotice: 'NOTICE',
      severityInfo: 'INFO',
      severityDebug: 'DEBUG',
      hostname: 'Hostname Override',
      hostnameHelp: 'Empty = use device hostname'
    },
    notify: {
      title: 'Event Notifications',
      channels: 'Active Channels',
      channelWebhook: 'Webhook',
      channelTelegram: 'Telegram',
      channelEmail: 'Email',
      cooldown: 'Cooldown (seconds)',
      cooldownHelp: 'Per event type, at most one notification is sent within this window',
      eventsSection: 'Event selection',
      eventsHelp: 'Only the events selected here trigger a notification. The test notification is always sent regardless.',
      eventsSelectAll: 'Select all',
      eventsSelectNone: 'Select none',
      eventsNoneWarning: 'No event is selected — nothing but the test notification will be sent.',
      eventEthLinkDown: 'Network link lost',
      eventEthLinkUp: 'Network link restored',
      eventRfModuleLost: 'Radio module stopped responding',
      eventRfModuleDetected: 'Radio module detected',
      eventMqttDisconnected: 'MQTT connection lost',
      eventMqttReconnected: 'MQTT connection restored',
      eventFactoryReset: 'Factory reset triggered',
      eventRestart: 'Restart triggered',
      eventCcuConnected: 'CCU connected',
      eventCcuDisconnected: 'CCU disconnected',
      eventLowHeap: 'Low free memory',
      webhookSection: 'Webhook',
      webhookUrl: 'URL',
      webhookSecret: 'Secret',
      telegramSection: 'Telegram',
      telegramToken: 'Bot Token',
      telegramChatId: 'Chat ID',
      smtpSection: 'Email (SMTP)',
      smtpServer: 'Server',
      smtpPort: 'Port',
      smtpTls: 'Encryption',
      smtpTlsNone: 'None',
      smtpTlsStarttls: 'STARTTLS',
      smtpTlsImplicit: 'TLS (implicit)',
      smtpUser: 'User',
      smtpPassword: 'Password',
      smtpFrom: 'From',
      smtpTo: 'To',
      secretPresent: '✓ Stored – retype to replace'
    },
    diag: {
      unsupported: 'Unknown diagnostic target',
      checkmk: {
        disabled: 'CheckMK is disabled',
        listening: 'CheckMK agent listening on TCP port {port}',
        not_ready: 'CheckMK is enabled but listener is not ready'
      },
      mqtt: {
        disabled: 'MQTT is disabled',
        tcp_ok: 'TCP connection to {host}:{port} succeeded',
        tcp_failed: 'TCP connection to {host}:{port} failed',
        tls_note: ' (TLS enabled, cert validation not tested)'
      },
      prometheus: {
        disabled: 'Prometheus is disabled',
        listening: 'Prometheus exporter listening on TCP port {port}',
        not_ready: 'Prometheus is enabled but listener is not ready'
      },
      syslog: {
        disabled: 'Syslog forwarding is disabled',
        tcp_ok: 'TCP connection to {host}:{port} succeeded',
        tcp_failed: 'TCP connection to {host}:{port} failed'
      },
      notify: {
        disabled: 'Notifications are disabled',
        queued: 'Test notification queued for channels 0x{mask}'
      }
    }
  },

  // About Page
  about: {
    title: 'About',
    version: 'Version 2.2.5-Beta.6',
    fork: 'Modernized Fork',
    forkDescription: 'This version is a modernized fork by Xerolux (2025), based on the original HB-RF-ETH firmware. Updated to ESP-IDF 6.0, modern toolchains and current WebUI technologies (Vue 3, Vite, Pinia).',
    original: 'Original Author',
    firmwareLicense: 'The',
    hardwareLicense: 'The',
    under: 'is released under',
    description: 'HomeMatic BidCoS/HmIP LAN Gateway',
    author: 'Author',
    license: 'License',
    website: 'Website',
    documentation: 'Documentation',
    support: 'Support',
    githubRepository: 'GitHub Repository',
    copyrightXerolux: 'Copyright (c) 2025, Xerolux',
    copyrightOriginal: 'Copyright (c) 2022, Alexander Reinert'
  },

  // Third Party
  thirdParty: {
    title: 'Third party software',
    containsThirdPartySoftware: 'This software contains free third party software products used under various license conditions.',
    providedAsIs: 'The software is provided "as is" WITHOUT ANY WARRANTY.'
  },

  // Change Password
  changePassword: {
    title: 'Password change required',
    eyebrow: 'Security',
    subtitle: 'Secure your account',
    warningTitle: 'Required',
    requirementsTitle: 'Password requirements:',
    reqMinLength: 'At least 8 characters',
    reqLettersNumbers: 'Must contain uppercase, lowercase and numbers',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    changePassword: 'Change Password',
    changeSuccess: 'Password changed successfully',
    changeError: 'Error changing password',
    passwordMismatch: 'Passwords do not match',
    passwordTooShort: 'Password must be at least 8 characters long and contain uppercase, lowercase letters and numbers.',
    passwordRequirements: 'Must contain uppercase, lowercase and numbers',
    passwordsDoNotMatch: 'Passwords do not match',
    warningMessage: 'This is your first login or the password is still set to "admin". For security reasons, you must change the password.',
    success: 'Password changed successfully',
    newPasswordPlaceholder: 'Enter new password',
    confirmPasswordPlaceholder: 'Confirm new password',
    currentPasswordPlaceholder: 'Enter current password',
    currentPasswordRequired: 'Current password is required',
    currentPasswordIncorrect: 'The current password is incorrect.',
    sessionExpired: 'Your session has expired. Please sign in again.',
  },

  // Sponsor
  sponsor: {
    title: 'Support this Project',
    description: 'If you like this project and want to support its development, you can use one of the options below.',
    thanks: 'Thank you for your support!',
    teslaReferral: 'Tesla Referral'
  },

  // System Log
  systemlog: {
    subtitle: 'Filter, pause and export the live device log.',
    paused: 'Paused',
    live: 'Live',
    lines: 'lines',
    newEntries: 'new',
    pause: 'Pause',
    resume: 'Resume',
    manualScroll: 'Manual scroll',
    copy: 'Copy',
    searchPlaceholder: 'Search log output',
    allLevels: 'All levels',
    levelError: 'Error',
    levelWarning: 'Warning',
    levelInfo: 'Info',
    levelDebug: 'Debug',
    noMatches: 'No matching log lines',
    copiedVisible: 'Visible log copied',
    copiedLine: 'Log line copied',
    copyVisibleFailed: 'Could not copy log output',
    copyLineFailed: 'Could not copy log line',
    title: 'System Log',
    description: 'Live view of system log output with download capability.',
    liveLog: 'Live Log',
    autoScroll: 'Auto-scroll',
    enabled: 'Enabled',
    disabled: 'Disabled',
    enableFailed: 'Could not enable log buffer (not enough memory).',
    disabledMessage: 'Log capture is disabled to save memory. Turn on the switch to capture live logs.',
    refresh: 'Refresh',
    clear: 'Clear',
    download: 'Download',
    empty: 'No log entries yet.',
    crashTitle: 'Crash log recovered',
    crashHint: 'The device restarted after a watchdog reset or system error. This tail of the last log lines was saved just before the restart and will not be available again after closing.'
  },

  // Privacy
  privacy: {
    title: 'Privacy',
    updateCheck: 'The device never searches for updates on its own. Only when you trigger “Search for updates now” in the interface does it fetch a small version file (under 1 KB) from xerolux.github.io and compare it with the installed version. No device data is transmitted; the request carries only your connection\u2019s public IP address. Nothing is downloaded or installed — the firmware upload still happens directly from your browser to the device. Optional features (MQTT, syslog) only connect to their configured servers when enabled.'
  },
}
