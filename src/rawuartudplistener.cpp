        ...
        case 0:                                 // connect
            if (length == 5 && data[2] == 1) {  // protocol version 1
                ...
                if (data[3] == 0) {
                    ...
                    ESP_LOGE(TAG,
                             "Received raw-uart reconnect packet with invalid endpoint identifier %d, should be %d",
...
void RawUartUdpListener::sendMessage(unsigned char command, unsigned char *buffer, size_t len)
{
    ...
    if (len) memcpy(sendBuffer + 2, buffer, len);
...
{
    if (!atomic_load(&_connectionStarted)) return;
