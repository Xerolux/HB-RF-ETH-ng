...
void Settings::load() {
    ...
    if (nvs_get_str(handle, "ipv6Mode", _ipv6Mode, &len) != ESP_OK)
        strncpy(_ipv6Mode, "auto", sizeof(_ipv6Mode) - 1);

    len = sizeof(_ipv6Address);
    if (nvs_get_str(handle, "ipv6Address", _ipv6Address, &len) != ESP_OK)
        _ipv6Address[0] = 0;

    GET_INT(handle, "ipv6Prefix", _ipv6PrefixLength, 64);

    len = sizeof(_ipv6Gateway);
    if (nvs_get_str(handle, "ipv6Gateway", _ipv6Gateway, &len) != ESP_OK)
        _ipv6Gateway[0] = 0;

    len = sizeof(_ipv6Dns1);
    if (nvs_get_str(handle, "ipv6Dns1", _ipv6Dns1, &len) != ESP_OK)
        _ipv6Dns1[0] = 0;

    len = sizeof(_ipv6Dns2);
    if (nvs_get_str(handle, "ipv6Dns2", _ipv6Dns2, &len) != ESP_OK)
        _ipv6Dns2[0] = 0;
...
char *Settings::getAdminPassword() { return _adminPassword; }
...
bool Settings::getPasswordChanged() { return _passwordChanged; }

char *Settings::getHostname() { return _hostname; }

bool Settings::getUseDHCP() { return _useDHCP; }

ip4_addr_t Settings::getLocalIP() { return _localIP; }

ip4_addr_t Settings::getNetmask() { return _netmask; }

ip4_addr_t Settings::getGateway() { return _gateway; }

ip4_addr_t Settings::getDns1() { return _dns1; }

ip4_addr_t Settings::getDns2() { return _dns2; }

void Settings::setNetworkSettings(char *hostname, bool useDHCP, ip4_addr_t localIP, ip4_addr_t netmask,
                                  ip4_addr_t gateway, ip4_addr_t dns1, ip4_addr_t dns2) {
    // Validate hostname
...
int Settings::getDcfOffset() { return _dcfOffset; }
...
int Settings::getGpsBaudrate() { return _gpsBaudrate; }
...
timesource_t Settings::getTimesource() { return (timesource_t)_timesource; }

void Settings::setTimesource(timesource_t timesource) { _timesource = timesource; }

char *Settings::getNtpServer() { return _ntpServer; }
...
int Settings::getLEDBrightness() { return _ledBrightness; }
...
bool Settings::getCheckUpdates() { return _checkUpdates; }

void Settings::setCheckUpdates(bool checkUpdates) { _checkUpdates = checkUpdates; }
...
// IPv6 Setter
void Settings::setIPv6Settings(bool enableIPv6, char *ipv6Mode, char *ipv6Address, int ipv6PrefixLength,
                               char *ipv6Gateway, char *ipv6Dns1, char *ipv6Dns2) {
    ...
    if (ipv6PrefixLength < 1)
        ipv6PrefixLength = 1;
    if (ipv6PrefixLength > 128)
