...
void rate_limiter_init()
{
    ...
    initialized = true;
    ESP_LOGI(TAG, "Rate limiter initialized (max %d attempts per %d seconds)", MAX_LOGIN_ATTEMPTS,
...
bool rate_limiter_check_login(httpd_req_t *req)
{
    ...
    if (entry->attempt_count >= MAX_LOGIN_ATTEMPTS) {
        ...
        ESP_LOGW(TAG, "Rate limit exceeded for IP %d.%d.%d.%d (wait %lld seconds)", ip_bytes[0], ip_bytes[1],
                 ip_bytes[2], ip_bytes[3], remaining_time);
...
void rate_limiter_reset_ip(httpd_req_t *req)
{
    ...
    for (int i = 0; i < MAX_TRACKED_IPS; i++) {
        if (rate_limit_table[i].is_active && rate_limit_table[i].ip_address == client_ip) {
            ...
            uint8_t *ip_bytes = (uint8_t *)&client_ip;
            ESP_LOGD(TAG, "Reset rate limit for IP %d.%d.%d.%d", ip_bytes[0], ip_bytes[1], ip_bytes[2], ip_bytes[3]);
            break;
        }
    }
}
