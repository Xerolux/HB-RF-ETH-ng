    ...
    switch (_detectState) {
        case DETECT_STATE_START_BL:
            if ((frame.destination == HM_DST_COMMON && frame.command == HM_CMD_COMMON_ACK && frame.data_len == 12 &&
                ...
                _detectState = DETECT_STATE_START_APP;
                sem_give(_detectWaitFrameDataSemaphore);
            } else if ((frame.destination == HM_DST_COMMON && frame.command == HM_CMD_COMMON_ACK &&
                        frame.data_len == 14 && frame.data[0] == 1 &&
                        strncmp((char *)(frame.data + 1), "DualCoPro_App", 13) == 0) ||
                ...
                sendFrame(_detectMsgCounter++, HM_DST_COMMON, HM_CMD_COMMON_START_BL, NULL, 0);
                        ...
                        frame.data_len == 13 && frame.data[0] == 1 &&
                        strncmp((char *)(frame.data + 1), "HMIP_TRX_App", 12) == 0) ||
        ...
        case DETECT_STATE_GET_SGTIN:
            if (frame.destination == HM_DST_COMMON && frame.command == HM_CMD_COMMON_ACK && frame.data_len == 13 &&
                frame.data[0] == 1) {
                sprintf(_sgtin, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X", frame.data[1], frame.data[2],
                        frame.data[3], frame.data[4], frame.data[5], frame.data[6], frame.data[7], frame.data[8],
                        frame.data[9], frame.data[10], frame.data[11], frame.data[12]);

                switch (_radioModuleType) {
                    case RADIO_MODULE_RPI_RF_MOD:
                        _bidCosRadioMAC = 0xff0000 | (frame.data[11] << 8) | frame.data[12];
                        if (_bidCosRadioMAC == 0xffffff) _bidCosRadioMAC = 0xfffffe;
                        sprintf(_serial, "%02X%02X%02X%02X%02X", frame.data[8], frame.data[9], frame.data[10],
                                frame.data[11], frame.data[12]);
        ...
        case DETECT_STATE_GET_BIDCOS_RF_ADDRESS:
            if (frame.destination == HM_DST_LLMAC && frame.command == HM_CMD_LLMAC_ACK && frame.data_len == 4 &&
                ...
                if (radioMac != 0 && (radioMac & 0xffff) != 0xffff) _bidCosRadioMAC = radioMac;
                _detectState =
                    _radioModuleType == RADIO_MODULE_RPI_RF_MOD ? DETECT_STATE_FINISHED : DETECT_STATE_GET_SERIAL;
                sem_give(_detectWaitFrameDataSemaphore);
            } else if (frame.destination == HM_DST_LLMAC && frame.command == HM_CMD_LLMAC_ACK && frame.data_len == 1 &&
                       frame.data[0] == 0) {
                if (_radioModuleType == RADIO_MODULE_RPI_RF_MOD) _detectState = DETECT_STATE_FINISHED;
                sem_give(_detectWaitFrameDataSemaphore);
            }
            break;
