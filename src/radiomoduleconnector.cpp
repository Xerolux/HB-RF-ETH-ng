...
void RadioModuleConnector::start()
{
    ...
    // Priority 12: Must be above application tasks but below lwIP TCPIP task (18)
    // to avoid starving the network stack during radio traffic bursts.
...
void RadioModuleConnector::_serialQueueHandler()
{
    ...
    for (;;)
    {
        if (xQueueReceive(_uart_queue, (void *)&event, (TickType_t)portMAX_DELAY))
        {
            ...
            // Yield after every event to give other tasks CPU time
            taskYIELD();
        }
    }
...
{
    // Use portMAX_DELAY to ensure no radio frames are silently dropped.
    // The mutex is only held briefly (vector copy), so this won't block
    // the UART task for any significant time. Dropping frames causes
    // HomeMatic devices to not appear in RaspberryMatic.
